...
BLOCK DATA iniFile
  LOGICAL initialized
  INTEGER num_lines
  CHARACTER LINES(100)*25
  CHARACTER iniFilename*256
  COMMON /Options/ initialized, num_lines, LINES, iniFilename
...
SUBROUTINE getValue(section, kwd, value, error)
  ...
  INTEGER num_lines
  LOGICAL initialized
  CHARACTER(len=25),dimension(100) :: LINEs
  CHARACTER iniFilename*256
  COMMON /Options/ initialized, num_lines, LINES, iniFilename
  INTEGER MAXLINE
...
!KR modification. Remove text after semicolon(;)
  commentind=index(value,';',back=.true.)
  if(commentind.gt.0)then
    value=trim(value(1:commentind-1))
  end if
END
